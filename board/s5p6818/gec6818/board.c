...
static int bd_set_recovery_wipe_data(void)
{
	...
	miscpart = env_get_ulong("miscpart", 0, 6);
	snprintf(devpart, ARRAY_SIZE(devpart), "%d:%d", rootdev, miscpart);

	ret = blk_get_device_part_str("mmc", devpart, &desc, &info, 0);
	...
	ret = blk_dread(desc, info.start, blkcnt, buf);
	if (ret != blkcnt)
		return -1;
	...
	ret = blk_dwrite(desc, info.start, blkcnt, buf);
